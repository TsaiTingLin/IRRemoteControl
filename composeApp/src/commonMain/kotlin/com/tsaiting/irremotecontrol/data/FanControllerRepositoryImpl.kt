...
import com.tsaiting.irremotecontrol.data.data.FanCommand
...
class FanControllerRepositoryImpl(
    private val fireStore: FirebaseFirestore,
    private val ioDispatcher: CoroutineDispatcher = Dispatchers.IO
) : FanControllerRepository {
    override fun observePowerState(): Flow<Boolean> = fireStore.collection(
        PATH
    )
        .document(ID)
        .snapshots
    ...
    override suspend fun powerChange(power: Boolean) {
        updateState(FanCommand.POWER.id)
        updatePower(power)

    }

    override suspend fun rotate() {
        updateState(FanCommand.ROTATE.id)
    }
